...
console.log(csv)
    ...
    .domain([0, d3.max(csv, function(e) { return   (+e.Production_totale)+1; })])
    .range(d3.range(9));
    ...
    .domain([0, d3.max(csv, function(e) { return +e.Production_totale; })])
    .range([0, 9 * 20]);  
...
//        console.log(e);
    d3.select("#d" + e.code)
    .filter(function(d) { return e.annee == selectedyear })  // <== This line
    .style("stroke","white")
        .style("opacity","1")
    .style("fill", function(d) { return colors[quantile(+e.Production_totale)]; })
        .attr("class", function(d) { return "region q" + quantile(+e.Production_totale) + "-9"; })
